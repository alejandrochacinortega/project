 ...
 * can load view directly into the cache in a `script` tag, or by consuming the
 ...
 * view. Locals definition is a hash of local scope property to its source:
 *
 ...
 *
 * The compiler does not suspend the entire compilation to wait for view to be loaded because this
 * would result in the whole app "stalling" until all view are loaded asynchronously - even in the
 ...
 * own view or compile functions. Compiling these directives results in an infinite loop and a
 ...
 * for their async view to be resolved.
 *
 *
 * ### Transclusion
 *
...
function $CompileProvider($provide, $$sanitizeUriProvider) {
  ...
  this.$get = [
     ...
     * is responsible for inlining directive view as well as terminating the application
    ...
    function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn,
      ...
      for (var i = 0, ii = directives.length; i < ii; i++) {
        ...
        if (directiveValue = directive.scope) {
          ...
          // skip the check for directives with async view, we'll check the derived sync
          ...
          if (directiveValue == 'element') {
            ...
            childTranscludeFn = compile($template, transcludeFn, terminalPriority,
                                        replaceDirective && replaceDirective.name, {
                                          ...
                                          // - newIsolateScopeDirective or templateDirective - combining view with
 ...
 * view.  Refer {@link ng.$sceDelegateProvider#resourceUrlWhitelist
 ...
 * that the URLs used for sourcing Angular view are safe.  Refer {@link
 ...
 * - but some of your view are hosted on other domains you control such as
 ...
 * ```
 *
 * ## Impact on loading view
 *
 * This applies both to the {@link ng.directive:ngInclude `ng-include`} directive as well as
 * `templateUrl`'s specified by {@link guide/directive directives}.
 *
 * By default, Angular only loads view from the same domain and protocol as the application
 * document.  This is done by calling {@link ng.$sce#getTrustedResourceUrl
 * $sce.getTrustedResourceUrl} on the template URL.  To load view from other domains and/or
 ...
 * loaded.  This means that without the right CORS policy, loading view from a different domain
 * won't work on all browsers.  Also, loading view from `file://` URL does not work on some
 * browsers.
 *
 ...
 * view in `ng-include` from your application's domain without having to even know about SCE.
 * It blocks loading view from other domains or loading view over http from an https
 ...
 *
 * The general syntax in view is as follows:
 *
 ...
 * $sce.getTrustedResourceUrl} on it. To load view from other domains or protocols
 ...
 * @example
  <example module="includeExample" deps="angular-animate.js" animations="true">
    <file name="index.html">
     <div ng-controller="ExampleController">
       <select ng-model="template" ng-options="t.name for t in view">
