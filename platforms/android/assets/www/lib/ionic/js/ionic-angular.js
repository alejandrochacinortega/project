...
/**
 * @ngdoc method
 * @name $ionicConfigProvider#view.maxPrefetch
 * @description Sets the maximum number of view to prefetch from the templateUrls defined in
 ...
 * @usage
 * State view are cached automatically, but you can optionally cache other view.
 *
 * ```js
 ...
 *     $ionicConfigProvider.view.prefetch(false);
 *
 ...
 *         templateUrl: "tabs-view/tabs.html"
 *       })
 *       .state('tabs.home', {
 *         url: "/home",
 *         views: {
 *           'home-tab': {
 *             prefetchTemplate: false,
 *             templateUrl: "tabs-view/home.html",
...
function($http, $templateCache, $timeout) {
  ...
  function run() {
    ...
    while (i < 4 && (template = toCache.pop())) {
      // note that inline view are ignored by this request
      if (isString(template)) $http.get(template, { cache: $templateCache });
      i++;
    }
    // only preload 3 view a second
    if (toCache.length) {
      $timeout(run, 1000);
    }
  }
 ...
 *
 * The ionNavView directive is used to render view in your application. Each template
 ...
 *
 * Pages are loaded by the URLs given. One simple way to create view in Angular is to put
